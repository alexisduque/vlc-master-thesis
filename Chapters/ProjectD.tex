...
\begin{figure}[htbp]
    ...
    \rule{35em}{0.5pt}
    \caption{CPU load and repartitions with only Oxymeter application running. Process on the left are system thread.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \label{fig:cpu-load}
    \rule{35em}{0.5pt}
    \caption[CPU load and repartitions during the web browsing simulation]{CPU load and repartition during the web browsing simulation}
\end{figure}
...
\subsection{hardware}
The MCU used for the LED sriver is an STM32L0, exactly the same as for the receiver, because its development kit is low cost, and toolchain with software development environment can be shared. We exploited its 3.3V digital output to blink the emitter white LED.
...
The application package has been called respecting Java name convention : edu.upc.entel.wng.vlcAraModule and contains 3 classes :
